...
| Speaker tab | Who is intended to speak | Audio-follow, glow, zoom. |
...
They agree the card is true. They cannot both be satisfied by the same sentence. Your furniture only needs to know who is `ON AIR`. Exact name case is mandatory.

## Voice of the package

High furniture, low conversation. Network pictures, group-chat talk.

- Charcoal plates, off-white type, amber and teal as **tabs and 4–8 px rules**, never as shells.
- Opaque. No glass, glow, skew, gradient, hairline, or texture that a Twitch 480p encode will eat.
- Condensed display type for kickers and headlines. A screen sans with tabular figures for clock, ticker, prices.
...
Do not clone a forest-green F1 suite, a finance J-screen, or any licensed display face. The recommended OFL stack is Barlow Condensed 700 + Inter 600/700.

## What “done” looks like

A still of `split` with a post card, both host IDs, one `ON AIR` tab, chyron, ticker, bug, LIVE, and clock must read as a show at 1920×1080 **and** after a 480p downscale. A designer who has never seen the hosts should still know: what the object is, who is speaking, and what the land is.
